...
- 2026-04-21: Completed the final alignment pass by verifying the scenario root matches the documented layout, checking CLI help exposes `--strict-validation`, documenting strict validation as the recommended workflow for real household runs, and preventing tests from drifting onto archived baselines.
- 2026-04-21: Completed the follow-up cleanup pass by making the committed test baseline load explicit in loader regressions, splitting strict-validation coverage into direct stale-age, filename/version, and modeled-death cases, tightening the scenario-root whitelist regression, and removing roadmap-style README claims that were no longer needed for current repo truth.
- 2026-04-23: Pinned shared federal tax and Medicare defaults to published 2025 values, updated deterministic downstream expectations, added direct key-value regressions for the committed policy defaults, and documented the audited default thresholds and premiums in the README.